...
object UpdateManager {
   ...
   */
  private def transformConfig(transform: Config => Config, identity: UserIdentity) {
  ...
  def createFront(request: CreateFront, identity: UserIdentity) = {
    val newCollectionId = Collection.nextId
    transformConfig(Transformations.createFront(request, newCollectionId), identity)
    newCollectionId
  }

  def updateFront(id: String, newVersion: Front, identity: UserIdentity) {
    transformConfig(Transformations.updateFront(id, newVersion), identity)
  }

  def addCollection(frontIds: List[String], collection: Collection, identity: UserIdentity) = {
    val newCollectionId = Collection.nextId
    transformConfig(Transformations.updateCollection(frontIds, newCollectionId, collection), identity)
    newCollectionId
  }

  def updateCollection(id: String, frontIds: List[String], collection: Collection, identity: UserIdentity) {
