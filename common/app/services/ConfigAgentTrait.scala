...
trait ConfigAgentTrait extends ExecutionContexts with Logging {
  ...
  def getSeoData(path: String): Future[SeoData] = {
    lazy val seoDataFromConfig:   Future[SeoData] = Future.successful(getSeoDataFromConfig(path))
    lazy val seoDataFromPath:     Future[SeoData] = Future.successful(SeoData.fromPath(path))
    lazy val itemResponseForPath: Future[Option[ItemResponse]] = getSectionOrTagWebTitle(path).fallbackTo(Future.successful(None))
    ...
    for {
      ...
      val section:  Option[String] = sc.section.orElse(ir.flatMap(_.section.map(_.id))).orElse(sp.section)
