...
trait ParseCollection extends ExecutionContexts with QueryDefaults with Logging {
  ...
  val amazonClient: ApiClient =
    ApiClient("aws-frontend-store", "DEV", AmazonSdkS3Client(new AmazonS3Client(Configuration.aws.credentials.get)))
  ...
  def getCollection(id: String, config: Config, edition: Edition): Future[Collection] = {
    val collection: Future[Option[com.gu.facia.client.models.Collection]] =
      amazonClient.collection(id)
        ...
        println(s"Recovering From ID($id) $t")
        None
      }
    ...
    val executeDraftContentApiQuery: Future[Result] =
      config.contentApiQuery.map { x =>
        val f = executeContentApiQueryViaCache(x, edition)
        f.onComplete {
          case Success(s) => println(s"Success")
          case Failure(f) => println(s"Failure $f")
        }
        f
      }.getOrElse(Future.successful(Result.empty))
