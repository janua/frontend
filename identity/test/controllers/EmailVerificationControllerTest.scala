  ...
  "Given the verify method is called" - Fake {
    ...
    "when the api call succeeds" - {
      ...
      "should display the validation completed page" in {
        ...
        contentAsString(result) should include("Thankyou! Your email address has been validated.")
        contentAsString(result) should not include("Your email conformation link has expired")
        contentAsString(result) should not include("Sorry, this email conformation link is not recognised.")
    ...
    "when the api call returns token expired error" - {
      ...
      "should display the validation completed page" in {
        status(result) should be(OK)
        contentAsString(result) should include("Your email conformation link has expired")
        contentAsString(result) should include("Resend my verification email")
        contentAsString(result) should not include("Thankyou! Your email address has been validated.")
        contentAsString(result) should not include("Sorry, this email conformation link is not recognised.")
      }
    }
    ...
    "when the api call returns invalid token error" - {
      ...
      "should display the validation completed page" in {
        status(result) should be(OK)
        contentAsString(result) should include("Sorry, this email conformation link is not recognised.")
        contentAsString(result) should include("Resend my verification email")
        contentAsString(result) should not include("Thankyou! Your email address has been validated.")
        contentAsString(result) should not include("Your email conformation link has expired")
      }
