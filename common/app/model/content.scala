...
class Content protected (val apiContent: ApiContentWithMeta) extends Trail with MetaData {
  ...
  override lazy val imageAdjust: String = apiContent.metaData.flatMap(_.imageAdjust).getOrElse("default")
  ...
  override lazy val showMainVideo: Boolean = false
      ...
      Option(
        Content(ApiContentWithMeta(
          ...
          supporting = (json \ "meta" \ "supporting").asOpt[List[JsValue]].getOrElse(Nil)
            .flatMap(Content.fromPressedJson),
          metaData = {
            val j = (json \ "meta").asOpt[TrailMetaData]
            val x = (json \ "meta").validate[TrailMetaData]
            println(x)
            println(s"Parsing out $itemId with result ${j.isDefined}")
            j
          }
        )
        )
